...
function! s:ExecLoad(option, long_option, pattern)
    ...
    if g:Gtags_No_Auto_Jump == 1
        ...
        " ajch: Add amibious processing
        if len(l:result) == 1
            " ajch: one result, just jump there
            cexpr! l:result
        else
            " ajch: Ambigious case, do NOT auto jump
            cgete l:result
        endif
    endif
    let &efm = l:efm_org
...
"
" ajch: Find the definition(s)
"
function! s:GtagsCursorDef()
    let l:pattern = expand("<cword>")
    let l:option = ""
    call s:ExecLoad('', l:option, l:pattern)
endfunction
...
"
" ajch: find the references
"
function! s:GtagsCursorRef()
    let l:pattern = expand("<cword>")
    let l:option = "--reference"
    call s:ExecLoad('', l:option, l:pattern)
endfunction
...
command! -nargs=0 GtagsCursor call s:GtagsCursorDef() 
command! -nargs=0 GtagsCursorRef call s:GtagsCursorRef()
command! -nargs=0 GtagsCursorDef call s:GtagsCursorDef()
